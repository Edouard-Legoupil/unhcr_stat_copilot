...
def _generate_data_visualization_code(
    ...
    code_lines.append("def get_valid_numeric_columns(df, numeric_cols, min_valid_count=1):")
    ...
    code_lines.append("            # Need at least min_valid_count non-null values and some variation")
    code_lines.append(f"            if non_null_count >= {min_valid_count} and unique_count >= 1:")
    ...
    if isinstance(actual_data, list) and len(actual_data) > 0:
        ...
        if numeric_cols:
            ...
            if year_col and len(numeric_cols) > 1:
                ...
                code_lines.append(f"    valid_numeric_cols = get_valid_numeric_columns(df, numeric_cols_for_plot, min_valid_count=2)")
                ...
                code_lines.append(f"    valid_cols = get_valid_numeric_columns(df, {numeric_cols}, min_valid_count=1)")
