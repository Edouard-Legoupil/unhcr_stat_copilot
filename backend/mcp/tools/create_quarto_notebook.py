    ...
    if isinstance(actual_data, list) and len(actual_data) > 0:
        ...
        if numeric_cols:
            ...
            if year_col and len(numeric_cols) > 1:
                ...
                code_lines.append("            # Get valid numeric columns, excluding the year column and identifier fields")
                code_lines.append(f"            recent_numeric = recent_data.select_dtypes(include=['number']).drop(columns=['{year_col}'], errors='ignore')")
                # Add exclusion of identifier columns
                code_lines.append("            # Exclude identifier columns like coo_id, coa_id, etc.")
                code_lines.append("            exclude_ids = ['coo_id', 'coa_id', 'id', '_id', 'iso']")
                code_lines.append("            for id_col in exclude_ids:")
                code_lines.append("                if id_col in recent_numeric.columns:")
                code_lines.append("                    recent_numeric = recent_numeric.drop(columns=[id_col], errors='ignore')")
                ...
                code_lines.append("                    # Create label mapping for x-axis")
                code_lines.append("                    field_labels = {")
                # Add field labels for all potential numeric columns
                for col in numeric_cols:
                    if col != year_col:
                        label = _get_field_label(col)
                        code_lines.append(f"                        '{col}': '{label}',")
                code_lines.append("                    }")
                code_lines.append("                    label_list = [field_labels.get(col, col) for col in valid_recent_cols]")
                code_lines.append("                    fig, ax = plt.subplots(figsize=(10, 5))")
                code_lines.append("                    ax.bar(label_list, recent_numeric[valid_recent_cols].iloc[0], color='#0072BC')")
                ...
                for col in numeric_cols:
                    ...
                    code_lines.append(f"                '{col}': '{label}',")
                code_lines.append("            }")
                code_lines.append("            label_list = [field_labels.get(c, c) for c in final_cols]")
                code_lines.append("            # Also exclude identifier columns from final_cols if they got through")
                code_lines.append("            final_cols_clean = [c for c in final_cols if not any(id_prefix in c for id_prefix in ['_id', 'id_', 'iso'])]")
                code_lines.append("            sums_clean = [sums[final_cols.index(c)] for c in final_cols_clean] if final_cols_clean else []")
                code_lines.append("            label_list_clean = [field_labels.get(c, c) for c in final_cols_clean]")
                code_lines.append("            fig, ax = plt.subplots(figsize=(12, 6))")
                code_lines.append("            # Use UNHCR primary blue for bars")
                code_lines.append("            colors = ['#0072BC'] * len(final_cols_clean)")
                code_lines.append("            ax.bar(range(len(final_cols_clean)), sums_clean, color=colors)")
                code_lines.append("            ax.set_xticks(range(len(final_cols_clean)))")
                code_lines.append("            ax.set_xticklabels(label_list_clean, rotation=45, ha='right')")
