...
def _generate_data_visualization_code(
    ...
    code_lines.append("        if len(x_values.unique()) < 1:")
    code_lines.append("            return False, f\"X-axis column '{x_col}' has no variation (all same value)\"")
    ...
    code_lines.append("    if len(values.unique()) < 1:")
    code_lines.append("        return False")
    ...
    if isinstance(actual_data, list) and len(actual_data) > 0:
        ...
        if numeric_cols:
            ...
            if year_col and len(numeric_cols) > 1:
                ...
                code_lines.append(f"    is_valid, message = has_valid_data(df, numeric_cols_for_plot, year_col='{year_col}')")
                ...
                code_lines.append("                # Check if we have valid numeric data")
                code_lines.append("                if len(recent_numeric.columns) > 0 and not recent_numeric.iloc[0].isna().all():")
                ...
                code_lines.append("                    plt.show()")
                code_lines.append("                else:")
                code_lines.append("                    print('No valid numeric data for recent year bar chart')")
                code_lines.append("            else:")
                ...
                code_lines.append("try:")
                code_lines.append("    # Validate data before plotting")
                code_lines.append(f"    is_valid, message = has_valid_data(df, {numeric_cols})")
                code_lines.append("    if not is_valid:")
                code_lines.append("        print(f'Cannot create distribution plot: {message}')")
                code_lines.append("    else:")
                code_lines.append("        fig, ax = plt.subplots(figsize=(12, 6))")
                code_lines.append(f"        valid_cols = [c for c in {numeric_cols} if c in df.columns]")
                code_lines.append("        if len(valid_cols) > 0:")
                code_lines.append("            # Use UNHCR primary blue for bars")
                code_lines.append("            colors = ['#0072BC'] * len(valid_cols)")
                code_lines.append(f"            ax.bar(range(len(valid_cols)), [df[c].sum() for c in valid_cols], color=colors)")
                code_lines.append(f"            ax.set_xticks(range(len(valid_cols)))")
                code_lines.append(f"            ax.set_xticklabels(valid_cols, rotation=45, ha='right')")
